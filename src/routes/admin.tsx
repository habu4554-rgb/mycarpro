...
function BlockedDates() {
  const [dates, setDates] = useState<{ id: string; blocked_date: string; reason: string | null }[]>([]);
  ...
  const load = async () => {
    ...
    else setDates((data ?? []) as typeof dates);
  };
  ...
  return (
    <section className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-soft">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Mark days the workshop is closed — clients cannot book on these dates.
      </p>

      <form onSubmit={block} className="mt-4 grid gap-3 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={inputCls} required />
        <input
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          placeholder="Reason (optional) — holiday, staff training…"
          className={inputCls}
        />
        <button
          type="submit"
          disabled={busy || !date}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-brand px-5 py-2.5 text-xs font-semibold text-primary-foreground shadow-brand hover:brightness-110 transition disabled:opacity-60"
        >
      ...
      {err && (
        <p className="mt-3 rounded-xl border border-destructive/40 bg-destructive/10 px-4 py-2.5 text-sm text-destructive">
          {err}
        </p>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {dates.map((d) => (
          <span
            key={d.id}
            className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-3.5 py-1.5 text-xs font-semibold"
          >
            {d.blocked_date}
            {d.reason && <span className="font-normal text-muted-foreground">· {d.reason}</span>}
            <button
              type="button"
              onClick={() => void unblock(d.id)}
              disabled={busy}
              aria-label={`Unblock ${d.blocked_date}`}
